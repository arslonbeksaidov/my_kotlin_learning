...
fun main() {
   ...
   fun makeTickTacMatrix() : MutableList<MutableList<String>>{
       val items2 = items
       items2.removeIf { it == "" }
       var ticTacMatrix = mutableListOf<MutableList<String>>()
       var temperoryList = mutableListOf<String>()
       for (item in 0..8) {
           if (item in 0..2) {
               temperoryList.add(items2[item])
               if (item == 2) {
                   ticTacMatrix.add(temperoryList)
                   temperoryList = mutableListOf()
               }
           }
           if (item in 3..5) {
               temperoryList.add(items2[item])
    ...
    fun drowGame(updatedItems: MutableList<MutableList<String>>) {
        println("---------")
        for (x in 0..2) {
            updatedItems[x].add(0, "|")
            updatedItems[x].add("|")
            println(updatedItems[x].joinToString(" "))
        }
        println("---------")
    }
    drowGame(makeTickTacMatrix())
    fun checkCases() {
        items.removeIf { it == "" }
    ...
    while (true)
        try {
            var ticTacMatrix = makeTickTacMatrix()
            println("Enter the coordinates: ")
            val corditanes = readLine()!!.split(" ").map { it.toInt() }.toList()
